...
export function DocumentCopilotPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const chooseFile = (next: File | undefined) => {
    ...
    if (next.size > 4 * 1024 * 1024) {
      setError("The file must be smaller than 4 MB.");
      return;
    }
    setError("");
    setFile(next);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "This document could not be analyzed.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <section className="border-b border-border bg-paper">
        <div className="mx-auto max-w-6xl px-6 py-9">
          ...
          <div className="mt-7 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-gold-600"><FileSearch className="h-4 w-4" /> Document &amp; Notice Copilot</p>
              <h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-tight text-ink sm:text-5xl">Turn confusing paperwork into clear next steps.</h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-haze">Compass identifies useful details, deadlines, and possible checklist matches. You decide what is correct before anything enters your case file.</p>
            </div>
            <div className="flex items-start gap-2 rounded-2xl border border-emerald-100 bg-mint/50 px-5 py-3 text-sm text-haze"><Lock className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />Your file is analyzed once and not stored.</div>
          </div>
        </div>
      </section>
        ...
        {!analysis ? (
          <div className="grid gap-6 lg:grid-cols-[1fr_21rem] lg:items-start">
            <section className="rounded-[2rem] border border-border bg-paper p-6 shadow-soft sm:p-8">
              ...
              <div className="mt-8">
                ...
                <div onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} className={cn("mt-4 rounded-[1.75rem] border-2 border-dashed p-10 text-center transition sm:p-14", dragging ? "border-gold-500 bg-gold-50" : "border-gold-300/60 bg-gold-50/40")}>
                  ...
                  {file ? <><h2 className="mt-5 font-display text-xl font-semibold text-ink">{file.name}</h2><p className="mt-2 text-sm text-haze">{(file.size / 1024 / 1024).toFixed(2)} MB · ready to review</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Button onClick={runAnalysis} disabled={loading || programIds.length === 0}>{loading ? <Loader2 className="animate-spin" /> : <FileSearch />}{loading ? "Reading document" : "Analyze document"}</Button><Button variant="outline" onClick={reset}><X />Remove</Button></div></> : <><h2 className="mt-5 font-display text-xl font-semibold text-ink">Drop a document here</h2><p className="mt-2 text-sm text-haze">PDF, JPEG, PNG, or WebP · up to 4 MB</p><Button variant="outline" className="mt-6" onClick={() => inputRef.current?.click()}><UploadCloud />Choose a file</Button></>}
                </div>
              </div>
            ...
            <aside className="rounded-[2rem] border border-emerald-100 bg-gradient-to-b from-mint/70 to-paper p-6 shadow-soft">
              ...
              <h2 className="mt-5 font-display text-2xl font-semibold text-ink">You stay in control.</h2>
              <ul className="mt-5 space-y-4 text-sm leading-6 text-haze">{["Compass shows the exact text behind each extracted detail.", "Sensitive fields are clearly marked.", "No profile or eligibility result changes automatically.", "Agency instructions should still be confirmed with the agency."].map((item) => <li key={item} className="flex gap-3"><Check className="mt-1 h-4 w-4 shrink-0 text-emerald-600" />{item}</li>)}</ul>
            </aside>
          </div>
        ) : (
