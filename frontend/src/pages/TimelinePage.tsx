...
export function TimelinePage({ navigate }: { navigate: (path: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (code && state) {
      ...
      setCalendarMessage("Google Calendar connected. Review the selected tasks before adding them.");
      window.history.replaceState({}, "", "/action-center/timeline");
    } else if (calendarError) {
      setCalendarMessage("Google Calendar was not connected. You can still download the calendar file.");
      window.history.replaceState({}, "", "/action-center/timeline");
    }
  }, []);
  ...
  const generate = async () => {
    if (!programIds.length && !acceptedAnalyses.some((analysis) => analysis.deadlines.length)) {
      setError("Choose at least one program or add a reviewed document with a deadline.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      ...
      setError(err instanceof Error ? err.message : "The timeline could not be built.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const connectCalendar = async () => {
    ...
    try {
      ...
      else setCalendarMessage("Google Calendar is not configured on this deployment. Download the calendar file instead.");
    } catch {
      setCalendarMessage("Google Calendar could not be connected.");
    }
  };
  ...
  const syncCalendar = async () => {
    ...
    try {
      ...
      setCalendarMessage(`${result.created} event${result.created === 1 ? "" : "s"} added${result.skipped ? ` · ${result.skipped} already existed` : ""}${result.errors.length ? ` · ${result.errors.length} failed` : ""}.`);
      setCalendarCode(null);
      setCalendarState(null);
    } catch (err) {
      setCalendarMessage(err instanceof Error ? err.message : "Calendar events could not be created.");
      setCalendarCode(null);
      setCalendarState(null);
    } finally {
      setLoading(false);
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-canvas pb-20">
    <section className="border-b border-border bg-paper"><div className="mx-auto max-w-6xl px-6 py-9"><button type="button" onClick={() => navigate("/action-center")} className="inline-flex items-center gap-2 text-sm font-semibold text-emerald-700"><ArrowLeft className="h-4 w-4" />Back to Action Center</button><div className="mt-7 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-lilac"><CalendarDays className="h-4 w-4" />Smart Timeline</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-tight text-ink sm:text-5xl">Give every next step a place and a date.</h1><p className="mt-4 max-w-2xl leading-7 text-haze">Compass combines selected programs, reviewed deadlines, and saved locations. Suggested dates stay visibly different from dates found in your documents.</p></div>{timeline && <Button variant="outline" onClick={() => setTimeline(null)}><RefreshCw />Rebuild plan</Button>}</div></div></section>

    <div className="mx-auto max-w-6xl px-6 py-8">
      {!timeline ? <div className="grid gap-6 lg:grid-cols-[1fr_21rem] lg:items-start"><section className="rounded-[2rem] border border-border bg-paper p-7 shadow-soft"><p className="text-xs font-bold uppercase tracking-[0.16em] text-haze">What should this plan include?</p><div className="mt-5 flex flex-wrap gap-2">{Object.entries(PROGRAMS).map(([id]) => { const active = programIds.includes(id); return <button key={id} type="button" onClick={() => setProgramIds((current) => active ? current.filter((value) => value !== id) : [...current, id])} className={cn("rounded-full border px-4 py-2 text-sm font-bold uppercase transition", active ? "border-lilac bg-lilac text-white" : "border-border bg-canvas text-haze")}>{id}</button>; })}</div><label className="mt-8 block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-haze">When would you like to submit the first application?</span><input type="date" min={dateAfter(0)} value={targetDate} onChange={(event) => setTargetDate(event.target.value)} className="mt-3 h-12 w-full max-w-xs rounded-xl border border-border bg-canvas px-4 font-semibold text-ink outline-none focus:border-lilac" /></label><div className="mt-8 grid gap-3 sm:grid-cols-2"><InputSummary icon={<FileCheck2 />} value={acceptedAnalyses.length} label="reviewed documents" detail={`${acceptedAnalyses.reduce((sum, item) => sum + item.deadlines.length, 0)} confirmed dates`} /><InputSummary icon={<MapPin />} value={center.savedResources.length} label="saved locations" detail={center.savedResources[0]?.name ?? "No location selected"} /></div>{error && <p className="mt-5 rounded-xl bg-clay/10 p-4 text-sm font-medium text-clay">{error}</p>}<Button size="lg" className="mt-7" onClick={generate} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : <Sparkles />}{loading ? "Building timeline" : "Build my timeline"}</Button></section><aside className="rounded-[2rem] border border-lilac/15 bg-lilac/5 p-6"><CalendarCheck2 className="h-7 w-7 text-lilac" /><h2 className="mt-5 font-display text-2xl font-semibold text-ink">Dates stay honest.</h2><div className="mt-5 space-y-4"><Legend color="bg-gold-500" title="Extracted" body="Found in a document you reviewed." /><Legend color="bg-emerald-500" title="Official" body="Published by a program or agency." /><Legend color="bg-lilac" title="Suggested" body="A planning date you can freely change." /></div></aside></div> : <div className="grid gap-6 lg:grid-cols-[1fr_21rem] lg:items-start"><section><div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-lilac">Your working plan</p><h2 className="mt-1 font-display text-3xl font-semibold text-ink">{timeline.tasks.length} next steps</h2></div><p className="text-sm text-haze">{timeline.tasks.filter((task) => task.completed).length} completed</p></div><div className="space-y-3">{timeline.tasks.map((task) => <TaskRow key={task.id} task={task} selected={selectedTaskIds.has(task.id)} onSelect={() => { const next = new Set(selectedTaskIds); next.has(task.id) ? next.delete(task.id) : next.add(task.id); setSelectedTaskIds(next); }} onComplete={() => updateTasks(timeline.tasks.map((item) => item.id === task.id ? { ...item, completed: !item.completed } : item))} onDate={(date) => updateTasks(timeline.tasks.map((item) => item.id === task.id ? { ...item, due_at: date, date_source: "suggested" } : item))} />)}</div></section><aside className="space-y-4 lg:sticky lg:top-24"><div className="rounded-[1.75rem] border border-border bg-paper p-6 shadow-soft"><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Calendar</p><h2 className="mt-2 font-display text-2xl font-semibold text-ink">Keep the plan moving.</h2><p className="mt-3 text-sm leading-6 text-haze">{selectedTasks.length} selected task{selectedTasks.length === 1 ? "" : "s"}. Review them before creating external events.</p><Button variant="outline" className="mt-5 w-full" onClick={() => downloadTimeline(selectedTasks)} disabled={!selectedTasks.length}><Download />Download calendar file</Button>{calendarCode && calendarState ? <Button className="mt-2 w-full" onClick={syncCalendar} disabled={loading || !selectedTasks.length}>{loading ? <Loader2 className="animate-spin" /> : <CalendarCheck2 />}Confirm Google Calendar events</Button> : <Button variant="soft" className="mt-2 w-full" onClick={connectCalendar}>{calendarConfigured ? <CalendarDays /> : <ExternalLink />}{calendarConfigured ? "Connect Google Calendar" : "Calendar setup required"}</Button>}{calendarMessage && <p className="mt-4 rounded-xl bg-mint p-3 text-xs leading-5 text-haze">{calendarMessage}</p>}<p className="mt-4 text-[11px] leading-5 text-haze">Calendar writes happen only after you select tasks and confirm here.</p></div><Button variant="outline" className="w-full" onClick={() => navigate("/action-center/resources")}><Building2 />Review saved places</Button></aside></div>}
    </div>
  </div>;
}
...
function TaskRow({ task, selected, onSelect, onComplete, onDate }: { task: ActionTask; selected: boolean; onSelect: () => void; onComplete: () => void; onDate: (date: string) => void }) { return <article className={cn("grid gap-4 rounded-2xl border bg-paper p-5 shadow-soft transition sm:grid-cols-[auto_1fr_auto] sm:items-start", task.completed ? "border-border opacity-60" : "border-border", selected && "ring-2 ring-emerald-100")}><button type="button" onClick={onComplete} className="mt-0.5 text-emerald-600" aria-label={task.completed ? "Mark incomplete" : "Mark complete"}>{task.completed ? <CheckCircle2 className="h-6 w-6" /> : <Circle className="h-6 w-6" />}</button><div><div className="flex flex-wrap items-center gap-2"><h3 className={cn("font-display text-lg font-semibold text-ink", task.completed && "line-through")}>{task.title}</h3>{task.program_id && <span className="rounded-full bg-canvas px-2 py-0.5 text-[10px] font-bold uppercase text-haze">{task.program_id}</span>}<span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold uppercase", sourceStyles[task.date_source])}>{task.date_source}</span></div><p className="mt-2 text-sm leading-6 text-haze">{task.description}</p>{task.location && <p className="mt-2 flex items-center gap-1.5 text-xs font-medium text-haze"><MapPin className="h-3.5 w-3.5" />{task.location}</p>}</div><div className="flex items-center gap-3 sm:flex-col sm:items-end"><input type="date" value={task.due_at.slice(0, 10)} onChange={(event) => onDate(event.target.value)} className="rounded-lg border border-border bg-canvas px-3 py-2 text-xs font-bold text-ink" /><button type="button" onClick={onSelect} className={cn("flex items-center gap-1.5 text-xs font-bold", selected ? "text-emerald-700" : "text-haze")}><span className={cn("grid h-4 w-4 place-items-center rounded border", selected ? "border-emerald-600 bg-emerald-600 text-white" : "border-border text-transparent")}><Check className="h-2.5 w-2.5" /></span>{selected ? "Selected" : "Select"}</button></div></article>; }
